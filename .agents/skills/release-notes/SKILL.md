...
This repository contains a browser extension for ABW.by.

The extension helps users see car prices on ABW.by in a convenient currency instead of only BYN.

Known product behavior:

- converts prices from BYN to USD, EUR, or RUB
- can return prices back to BYN
- works on ABW.by pages with listings, car cards, similar listings, dealer blocks, and monthly payment text
...
- `Расширение стало надежнее находить цены на странице ABW.by.`
...
- `The extension should now detect more prices on ABW.by pages more reliably.`
...
- `Prices should keep updating even when ABW.by loads listings dynamically.`
...
- Убрали проблему с [ценами / курсами / попапом / страницами ABW.by] — [почему это важно].
...
- Removed an issue with [prices / rates / popup / ABW.by pages] — [why it matters].
...
```md
- **Больше цен на странице:** расширение стало надежнее находить цены в разных блоках ABW.by.
```

Good English:

```md
- **More prices detected:** the extension should now find prices more reliably across different ABW.by page blocks.
```
